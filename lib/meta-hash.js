...
/* E.164 sem o "+", com DDI do Brasil quando o número vier sem.
   Com `comDDI` (as páginas em inglês, 11/09/2026) o número chega com o
   código do país e nada é prefixado: "15551234567" é um celular dos EUA,
   não um brasileiro sem 55. */
export function normPhone(p, comDDI = false) {
  let d = String(p || "").replace(/\D/g, "").replace(/^0+/, "");
  if (d && !comDDI && !d.startsWith("55")) d = "55" + d;
  return d;
}
